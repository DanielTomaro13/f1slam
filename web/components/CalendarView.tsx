...
export default function CalendarView({ data }: { data: F1Data }) {
  ...
  const nextRound = useMemo<number | null>(() => {
    ...
    return rounds.find((r) => !isPast(r.raceDate))?.round ?? null;
  }, [rounds, season, data.currentSeason]);

  return (
    <div style={{ display: "grid", gap: 18 }}>
      {/* Season filter */}
      <div style={{ display: "flex", gap: 6, overflowX: "auto" }} className="scroll-x">
        {data.seasons.map((y) => {
      ...
      {rounds.length === 0 ? (
        ...
        >
          {rounds.map((r) => {
            ...
            const done = season < data.currentSeason || isPast(r.raceDate);
            return (
              <Link
                key={r.round}
                href={`/tracks/${r.circuitKey}`}
                className="card"
                style={{
                  padding: 0,
                  display: "block",
                  ...
                  opacity: done && !isNext && !r.winner ? 0.92 : 1,
                }}
              >
                {/* Track-map thumbnail */}
                ...
                <div style={{ padding: "0.8rem", display: "grid", gap: 4 }}>
                  ...
                  <div style={{ marginTop: 6 }}>
                    {r.winner ? (
                      <span style={{ color: "var(--gold)", fontFamily: "var(--font-cond)", fontSize: "1rem" }}>
                        🏆 {r.winner}
                      </span>
                    ) : isNext ? (
                      <span
